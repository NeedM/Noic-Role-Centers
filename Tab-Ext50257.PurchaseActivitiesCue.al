tableextension 50257 "Purchase Activities Cue" extends "Purchase Cue"
{
    fields
    {
        ...
        {
            CalcFormula = Count("Requisition Header" where("Document Type" = const(Purchase), Status = filter(Open)));
        ...
        {
            CalcFormula = Count("Requisition Header" where("Document Type" = const(Purchase), Status = filter(Released)));
        ...
        {
            CalcFormula = Count("Requisition Header" where("Document Type" = const(Stock), Status = filter(Open)));
